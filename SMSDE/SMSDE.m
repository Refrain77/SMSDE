...
% =========================================================================
function [x, xold, fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq,current_eval,res_det,F,cr,op3_pos,op_score,age,nums] = ...
    SMSDE( x,xold, fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq, xmin, xmax,  n,...
    PopSize, current_eval, I_fno,res_det,Printing,Max_FES, G_Max,iter,F,cr,op3_pos,op_score,age,nums)
...
[r1, r2, r3, r4, r5] =generate_random(PopSize);
...
elseif current_op == 3 
    nums(3) = nums(3) + 1;
    vi(op_3 == 1, :) = x(r1(op_3==1), :)+ F(op_3==1, ones(1, n)) .*(x(r3(op_3==1), :)-x(r2(op_3==1), :));
elseif current_op == 2
    nums(2) = nums(2) + 1;
    vi(op_3==1, : ) = x(r1(op_3==1), :)+ F(op_3==1, ones(1, n)) .*(x(r3(op_3==1), :)-x(r2(op_3==1), :))+F(op_3==1, ones(1, n)) .*(x(r4(op_3==1), :)-x(r5(op_3==1), :));
elseif current_op == 4
    nums(4) = nums(4) + 1;
    vi(op_3==1, : ) = x(op_3==1,:) + F(op_3==1, ones(1, n)) .* (bestx-x(op_3==1,:)) + F(op_3==1, ones(1, n)) .* (x(r1(op_3==1), :)-x(r3(op_3==1), :));
end 
%%    
...
%     vi(op_3==1, : ) = x(r1(op_3==1), :)+ F(op_3==1, ones(1, n)) .*(x(r3(op_3==1), :)-x(r2(op_3==1), :))+F(op_3==1, ones(1, n)) .*(x(r4(op_3==1), :)-x(r5(op_3==1), :));
% elseif current_op == 4
%      vi(op_3==1, : ) = bestx + F(op_3==1, ones(1, n)) .*(x(r1(op_3==1), :)-x(r3(op_3==1), :));
% elseif current_op == 5
%     vi(op_3==1, : ) = bestx + F(op_3==1, ones(1, n)) .*(x(r1(op_3==1), :)-x(r3(op_3==1), :))+F(op_3==1, ones(1, n)) .*(x(r2(op_3==1), :)-x(r4(op_3==1), :));
% elseif current_op == 4
%     vi(op_3==1, : ) = x(op_3==1,:) + F(op_3==1, ones(1, n)) .* (bestx-x(op_3==1,:)) + F(op_3==1, ones(1, n)) .* (x(r1(op_3==1), :)-x(r3(op_3==1), :));
% elseif current_op == 5
%     vi(op_3==1, : ) = x(op_3==1,:)+K(op_3==1, ones(1, n)) .*(x(r1(op_3==1), :)-x(op_3==1, :))+F_hat(op_3==1, ones(1, n)) .* (x(r3(op_3==1), :)-x(r2(op_3==1), :));   
...
[fitx, ind]=sort(fitx);
if fitx(1) >= bestold
    %% Current best fit does not improve
    %% Switch to the next operator
    op3_pos=op3_pos + 1;
    if op3_pos > 4
       op3_pos = 1;
    end
    ...
    %% Update the score of operator
    op_score(2,current_op) = op_score(2,current_op)+1;
    op_score = sortrows(op_score', -2)';
    current_op = op_score(1,1);
   
end
age = age(ind);
x=x(ind,:);
xold = xold(ind,:);
% worst_gp(end+1, :) = x(end, :);
